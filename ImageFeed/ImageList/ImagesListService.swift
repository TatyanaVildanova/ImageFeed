...
//  ImageFeed
//
//  Created by TATIANA VILDANOVA on 15.11.2023.
//
...
final class ImagesListService {
    ...
    func fetchPhotosNextPage() {
        ...
        let task = session.descTask(for: request){ [weak self] (result: Result<[PhotoResult], Error>) in
    ...
    func changeLike(
        ...
        let task = session.descTask(for: request) { [weak self] (result: Result<LikeResult, Error>) in
            DispatchQueue.main.async {
                guard let self = self else { return }
                switch result {
                case .success(let body):
